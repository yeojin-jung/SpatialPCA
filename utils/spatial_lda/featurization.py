...
def _featurize_cells(
    ...
    if len(neighborhood_cells.index) == 0:
        raise ValueError(
            "There are no neighbours to compute features from \
(try include_anchors = True)"
    ...
    neighborhoods = anchor_kdTree.query_ball_tree(
        neighborhood_kdTree, radius, p=2.0, eps=0.0
    )
...
def _featurize_sample(
    ...
    i, df = data
    sample_features = _featurize_cells(
        df,
        neighborhood_feature_fn,
        radius,
        is_anchor_col,
...
def featurize_samples(
    ...
    featurize_sample_fn = functools.partial(
        _featurize_sample,
        neighborhood_feature_fn=neighborhood_feature_fn,
        radius=radius,
        is_anchor_col=is_anchor_col,
        x_col=x_col,
        y_col=y_col,
        z_col=z_col,
        include_anchors=include_anchors,
    ...
    if n_processes is not None:
        with Pool(n_processes) as pool:
            ...
            all_sample_features = list(
                tqdm(pool.imap(featurize_sample_fn, sample_dfs.items()), total=total)
            )
    else:
...
def featurize_spleens(
    spleen_dfs, neighborhood_feature_fn, radius=100, n_processes=None
):
    return featurize_samples(
        spleen_dfs,
        neighborhood_feature_fn,
...
def make_nearest_neighbor_graph(
    sample_features, sample_dfs, sample_idx, x_col, y_col, z_col=None
):
...
def make_minimum_spaning_tree_mask(num_nodes, src_nodes, dst_nodes, edge_lengths):
    num_edges = len(src_nodes)
    adjacency_matrix = coo_matrix(
        (edge_lengths, (src_nodes, dst_nodes)), shape=(num_nodes, num_nodes)
    ).tocsr()
    edge_index = coo_matrix(
        (np.arange(num_edges), (src_nodes, dst_nodes)), shape=(num_nodes, num_nodes)
    ).tocsr()
...
def make_difference_matrix(num_nodes, src_nodes, dst_nodes):
    ...
    difference_matrix = coo_matrix(
        (values, (rows, cols)), shape=(num_edges, num_nodes)
    ).tocsr()
    return difference_matrix


def make_merged_difference_matrices(
    sample_features, sample_dfs, x_col, y_col, z_col=None, reduce_to_mst=True
):
    ...
    for sample_idx in set(sample_idxs):
        ...
        difference_matrix = make_difference_matrix(num_nodes, src_nodes, dst_nodes)
        if reduce_to_mst:
